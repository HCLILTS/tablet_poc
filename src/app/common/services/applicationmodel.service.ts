...
export class ApplicationmodelService {
  ...
  PlayerError = new BehaviorSubject<any>(false);
  ...
  constructor(router: Router, httpHandler: HttphandlerService, commonLoader: CommonloaderService,
    ...
    private PostmessagecommunicationService: PostmessagecommunicationService) {
    ...
    this.config = [
      ...
      ['/ntemp13_1', '/ntemp13_1ext', 0],
      ['/ntemp2_1', '/ntemp2_1ext', 0],
      ['/ntemp4_1', '/ntemp4_1ext', 0],
      ['/ntemp6_1', '/ntemp6_1ext', 0],
      ['/ntemp7_1', '/ntemp7_1ext', 0],
      ['/ntemp20_1', '/ntemp20_1ext', 0],
      ['/evatemp13', '/evatemp13ext', 0],
      ['/evatemp15_', '/evatemp15_ext', 0],
    ];
  ...
  activeBg:any = "";
  get initVal() {
    console.log('get initVal applcation model');
    return this.initValues.files[this.currentActive].startAt;
  }
  curruntActiveSegment(){
    
    let curruntSegmentInfo = this.initValues.files[this.currentActive];
    this.SegmentValue = {
      'segmentName':curruntSegmentInfo['_chapterName'],
      'shortCode':curruntSegmentInfo['_shortCode'],
      'description':curruntSegmentInfo['_description'],
      'activeIndex':this.currentActive
  };
    console.log('aaaaaaaaaaaaaa', this.initValues.files[this.currentActive]);
  ...
  set event(value: any) {
    ...
    //$$this.externalCommunication.hideNavigationBar.next(true);
    const data = {
  ...
  private initLoaded(data): void {
   ...
   // if (data.environment.lms.enabled) {
     if(data.environment.platform === "dotnet"){
      console.log('jyoti: this is running on dotnet');
   ...
   else if(data.environment.platform === "web"){
    ...
    }else if(data.environment.platform === "electron"){
     // debugger;
      console.log('jyoti: this is running on electron');
      this.PostmessagecommunicationService.startConnection();//sending playerready in this method

      this.dataHandler = this.externalCommunication;
      //todo: need methods for handling here
      this.dataHandler.loadData(data.environment.platform, this.listener.bind(this), this.baseLoaded.bind(this), this.baseFailed.bind(this));
    }  else if(data.environment.platform === "android"){
      console.log('shailesh: this is running on android');
      this.PostmessagecommunicationService.startConnection();//sending playerready in this method
      //this.dataHandler = this.dataLoader;
      this.dataHandler = this.externalCommunication;

      //to be removed later// inducing a fake open event
      //  let msg= {
      //    'method' : 'open',
      //    'data' : {
      //      "homePath":"home",
      //      "playerPreview":true,
      //      "sessionId":"e29ee52d-d2f0-6799-2bc8-13c95ed31999-1656979200000",
      //      "forwardEnabled":true,
      //      "videoType":'video/mp4',
      //      "files":[
      //        {"segmentId":"1",
      //        "file":"http://192.168.1.7:8088/EVA_CH09_D02_SEG12/",
      //        "startAt":"",
      //        "isCompleted":false,
      //        "nav":"false"}
      //       ]}
  ...
  public nextSection(): void {
    let abc = this.initValues.files;
    console.log('nextSection application model service');
    this.refernceStore.setTitleFlag(false);
    ...
    if (this.currentSection > this.contentCollection.collection.length - 1) {
      console.log('BEFORE nextCollection');
      this.nextCollection();
  ...
  public nextSectionCustom(segmentId): void {
    let abc = this.initValues.files;
    console.log('nextSectionCustom application model service'+ segmentId);
    this.refernceStore.setTitleFlag(false);
    this.segmentBeginvariable = false;
    this.currentSection=segmentId;
    if (!this.eventDone && this.lastQuesNT) {
      let observ = this._nextBtnEvent.observers[this._nextBtnEvent.observers.length - 1];
      this._nextBtnEvent.observers.splice(0, this._nextBtnEvent.observers.length);
      this._nextBtnEvent.observers.push(observ);
    ...
    if (this.currentSection >= this.contentCollection.collection.length - 1) {
      console.log('BEFORE nextCollection');
      this.nextCollection2(segmentId);
      this.isVideoPlayed = false;
    } else {
      this.runContent();
    }
  ...
  private nextCollection(): void {
    console.log('INSIDE nextCollection');
  ...
  private nextCollection2(id): void {
    console.log('INSIDE nextCollection');
    this.segmentBeginvariable = true;
  ...
  public getautoPlayEnableFlag() {
    return this.initValues.autoPlayEnabled;
  }
...
*/
  ...
  */
  ...
  public getAllSegmentDetails(){
    let filesLength = this.initValues.files.length;
    let fileArray=[];
    for(let i =0; i<filesLength; i++ ){
      fileArray.push(this.initValues.files[i])
    }
