import { Component, OnInit, ViewChild, ElementRef, OnDestroy, AfterViewChecked, ViewEncapsulation } from '@angular/core';
...
@Component({
  ...
  templateUrl: './Ntemplate4_1.component.html',
  styleUrls: ['./Ntemplate4_1.component.scss']
})
export class NTemplate4V1Component implements OnInit, OnDestroy, AfterViewChecked {
  ...
  selectedOption: any = {};
  matchHolderDisabled: boolean = true;
  leftmatchHolderDisable:boolean = false;
  rightmatchHolderDisable:boolean = false;
  empty_category_text:string;
  isoption_selected:boolean = false;
  isstoredindex: any;
  ...
  @ViewChild('emptyModalRef') emptyModalRef: any;
  ...
  ngOnInit() {
      ...
      this.appModel.postWrongAttempt.subscribe(() => {
          if (this.appModel.feedbackType == "fullyIncorrect" || this.appModel.feedbackType == "partialIncorrect" || this.appModel.feedbackType == "emptyCategory" ) {
              this.resetActivity();
          } 
 ...
 optionsState(){
     console.log("selected Option List length="+this.selectedOptList.length);
     if((this.videoReplayd) && (this.selectedOptList.length == this.questionObj.noOfOptions)){
  ...
  setData() {
      ...
      this.optionList = JSON.parse(JSON.stringify(this.fetchedcontent.options));
      ...
      this.optionsCount = this.fetchedcontent.optionCount;
      console.log("optionscount="+this.optionsCount);
      if(( this.optionsCount == 12 ) || ( this.optionsCount > 6 )){
          this.isquestionMax = true;
          this.isquestionMin = false;
      }else if(this.optionsCount == 6){
          this.isquestionMin = true;
          this.isquestionMax = false;
      }
      ...
      for (let i = 0; i < this.options.length; i++) {
          if(this.isquestionMax){
             this.options[i].isOpen = true;
          }
          if(this.isquestionMin){
              this.options[i].isOpen = true;
              this.options[i].imgsrc.style_class = this.options[i].imgsrc.style_class_min;
          }              
      }
      /*Start: Theme Implementation(Template Changes)*/
      this.controlHandler = {
  ...
  rightHolderHover(ref) {
    ref.style.cursor = 'pointer';
  }

  rightHolderLeave(ref) {
    ref.style.cursor = '';
  }

  optionHover(idx, opt) {
      this.optionRef.nativeElement.children[idx].classList.add("scaleInAnimation");
      this.optionRef.nativeElement.children[idx].style.cursor = "pointer";
  }

  optionLeave(idx, opt,event) {
       this.optionRef.nativeElement.children[idx].classList.add("scaleOutAnimation");
      setTimeout(() => {
          this.optionRef.nativeElement.children[idx].classList.remove("scaleInAnimation");
          this.optionRef.nativeElement.children[idx].classList.remove("scaleOutAnimation");
      }, 500)
  }

  playOptionHover(idx, opt,event) {
      this.isImageBorder = true;
        this.appModel.notifyUserAction();
  ...
  /**** function called on selecting an option */
  selectOption(idx, opt, event) {
      this.selectedOpt.idx = idx;
      this.isstoredindex = idx;
      this.selectedOption = opt;
      this.matchHolderDisabled = false;
      this.isoption_selected = true;      
      
      /*categoryA box container*/    
        let totalCategoryA = (this.categoryA.incorrect.length + this.categoryA.correct.length);         
         if(totalCategoryA == 8){             
             this.leftmatchHolderDisable = true;
          }
          else {
             this.leftmatchHolderDisable = false;
          }
       /*categoryB box container*/
        let totalCategoryB = (this.categoryB.incorrect.length + this.categoryB.correct.length);                    
         if(totalCategoryB == 8){
            this.rightmatchHolderDisable = true;
          }
          else{
            this.rightmatchHolderDisable = false;
          }

        for (let i = 0; i < this.options.length; i++) {
            /*option hover image applied*/     
          if( i == this.selectedOpt.idx ){
              this.options[i].imgsrc.url = this.options[i].mouse_over_img.url;//opt.mouse_over_img.url;
           }
           /*reassign appear in options image*/
           if(i != this.selectedOpt.idx){
               for(let j=0; j < this.optionList.length;j++){  
                   if(j != this.selectedOpt.idx) {                
                     this.options[j].imgsrc.url = this.optionList[j].imgsrc.url;
                   }                   
                }             
             }
  ...
  /**** function called on selecting the left and right match holder box */
  submitAttempt() {
      let idx = this.selectedOpt.idx;
      let opt = this.selectedOption;  

        /*remove border image*/
       for(let i=0;i<this.optionList.length;i++) {
           if(opt.index == i){
               opt.imgsrc.url = this.optionList[i].imgsrc.url;
           }
       }
      ...
      setTimeout(() => {
          ...
          if (this.leftSelectedIdx > 0 && this.rightSelectedIdx > 0) {
          ...
          if (this.selectableOpts > 0) {
          ...
          } else {    // All allowed options are selected
              ...
              if(this.isquestionMin){
                 this.mainContainer.nativeElement.classList = "bodyContentMin disableDiv"; 
              }
 ...
 resetOption(){
  ...
  checkforQVO() {
      ...
      if (this.questionObj && this.questionObj.quesInstruction && this.questionObj.quesInstruction.url && this.questionObj.quesInstruction.autoPlay) {
          ...
          this.quesVORef.nativeElement.onended = () => {
              ...
              if(this.isquestionMin){
                  this.mainContainer.nativeElement.classList = "bodyContentMin";
              }
              this.instructionDisable=false;
            //   this.startActivity();
    ...
    blinkCategoryA(randomIdx?) {      
      if (this.instructionVO && this.instructionVO.nativeElement.play) {
        ...
        /*show empty category box popup*/
          totalCategoryA+=1;
          if((totalCategoryA == this.quesObj.noOfOptions)){
            console.log("test empty popup");
            this.empty_category_text = this.infoPopupAssets.empty_cateb_text.url;
             setTimeout(() => {
              this.appModel.feedbackType = "emptyCategory";
              this.emptyModalRef.nativeElement.classList = "modal displayPopup";
             // clearInterval(this.blinkTimeInterval);             
             }, 300)
          }
    ...
    blinkCategoryB(randomIdx?) {
       ...
       if (this.instructionVO && this.instructionVO.nativeElement.play) {
        ...
        totalCategoryB+=1;
        if(totalCategoryB == this.quesObj.noOfOptions){
            this.empty_category_text = this.infoPopupAssets.empty_catea_text.url;
             setTimeout(() => {
              this.appModel.feedbackType = "emptyCategory";
              this.emptyModalRef.nativeElement.classList = "modal displayPopup";
              clearInterval(this.blinkTimeInterval);             
      ...
      if (this.isAllRight) {            
          ...
          if (this.isWrongAttempted) {
              ...
              if (this.categoryA && (this.categoryA.correct.length || this.categoryA.incorrect.length)) {
                  ...
                  if ((this.categoryA.correct.length == this.optionHolder.left_random_index.length) && (this.categoryA.incorrect.length == 0)) {
                      this.feedbackAssets.style_header = this.fetchedcontent.category_1.right_style_header;
                      this.feedbackAssets.style_body = this.fetchedcontent.category_1.right_style_body;
                      this.feedbackAssets.popTitleTxt_img = this.fetchedcontent.category_1.right_style_title;
                      this.popupTxtRequired=this.fetchedcontent.category_1.right_style_title.required;
                  ...
                  if ((this.categoryB.correct.length == this.optionHolder.right_random_index.length) && (this.categoryB.incorrect.length == 0)) {
                      this.feedbackAssets.style_header = this.fetchedcontent.category_2.right_style_header;
                      this.feedbackAssets.style_body = this.fetchedcontent.category_2.right_style_body;
                      this.feedbackAssets.popTitleTxt_img = this.fetchedcontent.category_2.right_style_title;
                      this.popupTxtRequired=this.fetchedcontent.category_2.right_style_title.required;
  ...
  getAnswer() {
      ...
      setTimeout(() => {
          ...
          this.emptyModalRef.nativeElement.classList = "modal";
