...
@Component({
	...
	styleUrls: ['./ntemplate14.component.scss']
})
export class Ntemplate14Component implements OnInit {
					...
					this.audioT.nativeElement.addEventListener('loadedmetadata', () => {
						...
						if (this.audioT.nativeElement.duration === Infinity) {
							...
							this.audioT.nativeElement.ontimeupdate = () => {
								...
								console.log("audioT duration="+this.audioT.nativeElement.duration);
	...
	ngOnInit() {
		...
		clearTimeout(this.replayTimer2);
	...
	playEventCallback() {
		this.appModel.stopAllTimer();
		//console.log("playeventCallback="+this.replayTimer);
		clearInterval(this.nextSegmenttimerId);
		clearTimeout(this.replayTimer);
		clearTimeout(this.lastPopUptimer);
		this.instructionDisable = true;
    ...
    autoplayRecording(){
	console.log("MediaRecorder State="+this.isPaused);
    if (this.isPaused){
      	this.replayTimer2 = setTimeout(() => {
		 console.log("replayTimer not clear");
		  if (this.audioT && this.audioT.nativeElement && this.isPlay) {
			  this.audioT.nativeElement.play();
		   }
		 },this.replayTime);
      }
	...
	pauseEventCallback() {
		...
		this.replayTimer2 = undefined;
		this.instructionDisable = false;     

		if (this.appModel.isLastSectionInCollection && !this.isFirstTrial) {
			console.log("Recording status =2"+this.recordingStatus);			
			this.handleTimer();
		}
