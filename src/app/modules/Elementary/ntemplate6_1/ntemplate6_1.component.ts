...
@Component({
  ...
  templateUrl: './ntemplate6_1.component.html',
  styleUrls: ['./ntemplate6_1.component.scss']
})
export class NTemplate6V1Component implements OnInit, OnDestroy, AfterViewInit, AfterViewChecked {
  ...
  @ViewChild('halfLetterRef') halfLetterRef: any;
  ...
  saveMatra: any;
  videoClose: boolean = false;
  halfMatraAssets:any;
  qvoPlayed:boolean=false;
  defaultLetterConfig = [
    {
      id: "L1",
  ...
  ngOnInit() {
    ...
    this.tempSubscription = this.appModel.getNotification().subscribe(mode => {
      if (mode == "manual") {
        ...
        this.halfLetterRef.nativeElement.classList = "modal";
    ...
    this.confirmPopupSubscription = this.appModel.getConfirmationPopup().subscribe((val) => {
      ...
      if (val == "uttarDikhayein") {
        ...
        for (let i = 0; i < this.optionObj.length; i++) {
           this.optionsClickable.nativeElement.children[0].children[i].children[2].pause();
           this.optionsClickable.nativeElement.children[0].children[i].children[2].currentTime = 0;     
        }
  ...
  OptionZoomOutAnimation(opt, i) {
    if (!this.checked && this.quesVORef.nativeElement.paused && this.instructionVO.nativeElement.paused) {
  ...
  checkAnswer(opt, id) {
    var isHalfLetterSelected = this.refQuesObj.quesID[this.index].halfLetter;
    if (isHalfLetterSelected) {       
       for (let i = 0; i < this.optionObj.length; i++) {
         if(!this.optionsClickable.nativeElement.children[0].children[i].children[2].paused) {
            this.optionsClickable.nativeElement.children[0].children[i].children[2].pause();
            this.optionsClickable.nativeElement.children[0].children[i].children[2].currentTime = 0;
       ...
       this.halfLetterRef.nativeElement.classList = "displayPopup modal";        
       this.disableSection = false;
       this.disableSpeaker = false;
    } else {
      ...
      if (opt.position == "left" || opt.position == "top" || opt.position == "bottom" || opt.position == "bottom_spcialCase") {
        this.refQuesCopy = this.QuesArr.slice();
        if (this.refQuesObj.quesID[this.index].isHalfLetterSpecialCase && this.optionsAssets[this.currentOptionNumber].optionID === 'M6') {
          this.refQuesCopy.splice(this.index - 1, 0, opt.id);
        } else {
      ...
      if (this.refQuesCopy.join('') == this.feedback.correct_ans_array.join('') && this.currentOptionNumberjson === this.index + 1) {
        this.flag = true;
  ...
  onClickAnimation(option, id) {
    ...
    if (option.position == "right") {
      ...
      if (this.refQuesObj.quesID[this.index].isHalfLetterSpecialCase && this.optionsAssets[this.currentOptionNumber].optionID === 'M6') {
        this.Matra.nativeElement.children[this.index - 1].insertAdjacentHTML("beforebegin", "<img style='opacity:0;height:78%;width:4%'></img>");
      } else {
  ...
  onClickAnimationManually(option, id, letterNumber) {
    ...
    console.log("start Animation");
    if (option.position == "right") {
      if (!this.flag || !this.isRightSelected) {
      ...
      if (!this.flag || !this.isRightSelected) {
        if (this.refQuesObj.quesID[letterNumber - 1].isHalfLetterSpecialCase && (this.isShowAnswer ? this.optionsAssets[this.currentMatraNumberjson].optionID === 'M6' : this.optionsAssets[this.currentOptionNumber].optionID === 'M6')) {
          this.Matra.nativeElement.children[letterNumber - 2].insertAdjacentHTML("beforebegin", "<img style='opacity:0;height:78%;width:4%;'></img>");
        } else {
    ...
    this.duplicateOption.nativeElement.children[id].style.left = this.percentLeft;
  ...
  onMouseMove(e) {
    ...
    if (this.clicked && this.videoClose) {
  ...
  hoverOk() {
    this.halfMatraAssets.ok_btn = this.halfMatraAssets.ok_btn_hover;
  }

  houthoverOk() {
    this.halfMatraAssets.ok_btn = this.halfMatraAssets.ok_btn_original;
  }
  ...
  setData() {
    if (this.appModel && this.appModel.content && this.appModel.content.contentData && this.appModel.content.contentData.data) {
      ...
      this.optionsAssets.forEach(option => {
        this.optionArr.push(option.optionID);
      })
      ...
      // this.refQuesObj.quesIdConfig.forEach(element => {
      //   for(let letterConfig of this.defaultLetterConfig) {
      //     if(element === letterConfig.id) {
      //       this.refQuesArr.push(letterConfig);
      //       break;
      //     }
      //   }
      // });
      ...
      this.halfMatraAssets = this.fetchedcontent.feedback.halfMatra_popup;
  ...
  closeModal() {
    ...
    if (this.flag) {
      ...
      if (!this.isRightSelected) {
        if (this.refQuesObj.quesID[this.currentOptionNumberjson - 1].isHalfLetterSpecialCase && this.optionsAssets[this.currentOptionNumber].optionID === 'M6') {
          this.Matra.nativeElement.children[this.currentOptionNumberjson - 2].remove();
        } else if ((this.optionsAssets[this.currentOptionNumber].optionID !== 'M12') && (this.optionsAssets[this.currentOptionNumber].optionID !== 'M13')) {
          this.Matra.nativeElement.children[this.currentOptionNumberjson].remove();
        }
      }
      this.mainContainer.nativeElement.style.opacity = "0.3";
