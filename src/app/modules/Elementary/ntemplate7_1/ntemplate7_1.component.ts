...
@Component({
    ...
    templateUrl: './ntemplate7_1.component.html',
    styleUrls: ['./ntemplate7_1.component.scss']
})
export class NTemplate7V1Component implements OnInit, OnDestroy, AfterViewChecked {
    ...
    blinkingQue: boolean = true;
    updateImage: boolean = true;
    quesObjCopyModal: any;
    ...
    ngOnInit() {
        ...
        this.tempSubscription = this.appModel.getNotification().subscribe(mode => {
            if (mode == "manual") {
                ...
                this.autoMode = true;
                this.blinkingQue = false;
                this.quesObjCopyModal = JSON.parse(JSON.stringify(this.questionObj));
                for (let i = 0; i < this.optionRef.nativeElement.children.length; i++) {
                    if (this.optionObj.opts[i].isCorrect) {
                        this.quesObjCopyModal.questionText[this.quesEmptyTxtIndx].url = this.optionObj.opts[i].popupImage.url;
                    }
                }
    ...
    setData() {
        ...
        console.log(this.feedbackObj, 'feedback object');
    ...
    blinkHolder() {
        if (this.blinkingQue) {
            this.blinkTimeInterval = setInterval(() => {
                if (this.updateImage && this.blinkingQue) {
                    for (let i = 0; i < this.quesObjCopy.questionText.length; i++) {
                        if (this.quesObjCopy.questionText[i].isHide) {
                            this.quesObjCopy.questionText[i].url = this.quesObj.blankSection.blink.url;
                    ...
                    if (this.blinkingQue) {
                        for (let i = 0; i < this.quesObjCopy.questionText.length; i++) {
                            if (this.quesObjCopy.questionText[i].isHide) {
                                this.quesObjCopy.questionText[i].url = this.quesObj.blankSection.default.url;
            ...
            console.log('blinking not required')
        }
    }
    startActivity() {
    }
    ...
    sendFeedback(ref, flag: string, action?: string) {
        ...
        if (action == "showAnswer") {
            ...
            if (this.autoMode) {
                for (let i = 0; i < this.optionRef.nativeElement.children.length; i++) {
                    if (this.optionObj.opts[i].isCorrect) {
                        this.quesObjCopy.questionText[this.quesEmptyTxtIndx].url = this.optionObj.opts[i].url;
                    }
                }
            } else {
                this.autoMode = false;
            ...
            if (this.showAnswerPopup) {
               if (!this.isOptionSelected) {
                    this.blinkingQue = false;
                    //this.refQues.nativeElement.children[0].children[this.quesEmptyTxtIndx].src = "";
                    //this.refQues.nativeElement.children[0].children[this.quesEmptyTxtIndx].classList = "dark engTxtMargin ansSpace adjustBlink";
    ...
    selectOpt(opt, idx) {
        ...
        if (this.optionRef && this.optionRef.nativeElement && this.optionRef.nativeElement.children[this.optionSelected].children[1]) {
            ...
            this.boundingClientTo = this.refQues.nativeElement.children[0].children[this.quesEmptyTxtIndx].getBoundingClientRect();
            opt.isOpen = true;
            ...
            if (opt && opt.isCorrect) {
                ...
                setTimeout(() => {
                    ...
                    this.quesObjCopyModal = JSON.parse(JSON.stringify(this.questionObj));
                    this.quesObjCopyModal.questionText[this.quesEmptyTxtIndx].url = opt.popupImage.url;
