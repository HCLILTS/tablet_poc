...
@Component({
    selector: 'app-template15_',
    templateUrl: './template15_.component.html',
    styleUrls: ['./template15_.component.scss']
})
export class Template15_Component implements OnInit ,OnDestroy,AfterViewChecked,AfterViewInit {
    blink: boolean = false;
    commonAssets: any = "";
    rightPopup: any;
    wrongPopup: any;
    ...
    selectedoption :any = {'id':null}
    lockOption(opt, index){
        console.log("opt",opt)
        ...
        //opt.image = opt.image_hover;
        /////////////
        if (opt && opt.audio && opt.audio.url) {
            //opt.image = opt.image_hover;
            this.playSound(opt.audio, index);
        }
