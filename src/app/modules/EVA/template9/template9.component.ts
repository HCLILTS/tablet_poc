...
export class Template9Component implements OnInit, AfterViewInit, AfterViewChecked, OnDestroy {
  ...
  onSVGClick(event) {
    ...
    if (questionIndex != -1) {
      this.quesObj.tablet.questionText.forEach((ques, index) => {
        ...
        if(!ques['answered'] && ques['selected']){
  ...
  getSVGLoaded(fileData) { 
    ...
    this.appModel.getFileString(fileUrl)
      .subscribe((data) => {
        const newNode:Document = new DOMParser().parseFromString(data, 'text/xml');  
        newNode.documentElement.style.maxWidth = '100%';
        newNode.documentElement.style.maxHeight = '100%';        
        var svgList = document.getElementById('svgContainer');   
           if (svgList.hasChildNodes()){              
              //console.log("Children Length=1"+svgList.children.length);
              svgList.removeChild(svgList.lastChild);               
              //console.log("Children Length=2"+svgList.children.length);
           }                  
         ...
         this.loaderTimer = setTimeout(() => {
          ...
          newNode.documentElement.removeChild(newNode.documentElement.lastChild);
          svgList.appendChild(newNode.documentElement);
      });
  }
  ...
  checkAnswer(option, index) { 
    ...
    if (option.id === this.quesObj.tablet.questionText[this.selectedIndex].correctAnsId) {
      ...
      let checkFlag;
      if((option.image_bg_color != "") && (option.image_bg_color != undefined)){         
        ...
        this.selectedColor = option.image_bg_color;
      }
  ...
  doRandomize(array) {
    ...
    while (0 !== currentIndex) {
      ...
      const img_audio1 = array[currentIndex].audio.url;
      ...
      const img_audio2 =array[randomIndex].audio.url;
      ...
      array[currentIndex].audio.url = img_audio1;
      ...
      array[randomIndex].audio.url = img_audio2;
