  ...
  checkAnswer(option, index) {
    ...
    if (currentIndexArr[this.selectedLetterCount] == option.id) {
      ...
      if (this.selectedLetterCount >= 0) {
        ...
        this.rightAnsBackground = option.image_bg_selected;//option.image_bg_original;
      }
       else {
        option.image_bg = option.image_bg_selected;//this.feedback.correct_ans[this.correctAnswerCounter]["image_bg"];
